...
if __name__ == '__main__':
    ...
    while True:
        ...
        # control loop
        i, j = back_points.shape[0]//2, back_points.shape[1]//2
        grasp_normal = np.array([0., 0., 1.])
        grasp_position = back_points[i, j].cpu().numpy()
        # grasp_orientation = get_orientation(config.init_quaternion, grasp_normal)
        d_init = config.init_cam_height - grasp_position[2]
        d = d_init
        ...
        while True:
            ...
            if d <= 0.1 + config.vacuum_length:
                ...
                # [x, y, z, x, y, z, w] = [position, quaternion]
                # position = surface point
                # quaternion : computed from normal
                ...
                T_cam2world[:3, 3] = grasp_position + grasp_normal * d
